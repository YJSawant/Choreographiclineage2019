	...
	if($count>0){
		...
		while($row = mysqli_fetch_array($artist_id_name_result)){
			...
			if($row['is_user_artist'] == "artist") {
				$node_border_array[] = array('id' => $row['artist_profile_id'], 'border_color' => '#da0067');
			}
			else {
				$node_border_array[] = array('id' => $row['artist_profile_id'], 'border_color' => '#025457');
			}
		}
	}
	//fetch edges
...
//fetch education nodes
	...
	$profile_details = $artist_details+$education_nodes;
	// $education_nodes.forEach(function(element){
	// 	$profile_details.push({
	// 	id:element.id,
	// 	date:element.date,
	...
	//$education_nodes.push(artist_details);
	...
	return array("nodes" => $nodes, "edges" => $edges, "node_borders" => $node_border_array,
	 "education_nodes" => $education_nodes, "artist_details" =>$artist_details,

...
function setupGUI(config) {
	...
	// relationsip
	$(".link1").next().html('Studied with');
	$(".link2").next().html('Collaborated with');
	$(".link3").next().html('Danced for');
	$(".link4").next().html('Influenced by');
...
function nodeActive(a) {
    ...
    if (f.attributes) {
        ...
        for (var attr in f.attributes) {
            ...
            if (attr =="img"){
                h = "<div><img src =/profiles/"+f.attributes["img"] +" style=\"vertical-align:middle\" /></div>"
            }
